...
def create_user_endpoint(user: UserCreate):
    u = create_or_get_user(user.username, user.email, user.password, user.age, user.bio)
    return {"user_id": u["id"]}
...
def like_endpoint(like: LikeAction):
    data, is_match, match_id = add_like(like.user_id, like.liked_user_id)
    return {"success": True, "is_match": is_match, "match_id": match_id}

# 5️⃣ Récupérer les matches (optionnel)
